        ...
        for attempt in range(max_retries + 1):
            ...
            if response.status_code == 200:
                ...
                if is_model_unavailable_error:
                    logger.warning(
                        f"[OpenAIProvider] Model '{clean_model}' returned HTTP {response.status_code} (model unavailable). "
                        "Marking model unavailable and routing to fallback."
                    )
                    registry.mark_model_unavailable(self.provider_name, clean_model)
                else:
                    logger.warning(
                        f"[OpenAIProvider] Model '{clean_model}' returned HTTP {response.status_code}: {response.text[:200]}. "
                ...
                async with httpx.AsyncClient(timeout=45.0) as client:
                    async with client.stream("POST", url, json=payload, headers=headers) as response:
                        if response.status_code == 429:
                            ...
                            if is_model_unavailable_error:
                                logger.warning(
                                    f"[OpenAIProvider] Streaming model '{clean_model}' returned HTTP {response.status_code} (model unavailable). "
                                    "Marking model unavailable and routing to fallback."
                                )
                                registry.mark_model_unavailable(self.provider_name, clean_model)
                            else:
                                logger.warning(
                                    f"[OpenAIProvider] Streaming model '{clean_model}' returned HTTP {response.status_code}: {error_body[:200]}. "
                                    "Routing to fallback without marking model unavailable."
                                )
                            await cb.record_failure("model_invalid", no_trip=True)  # bad model ID, not broken provider
                            metrics.record_call_failure(self.provider_name, clean_model, "model_invalid", response.status_code)
